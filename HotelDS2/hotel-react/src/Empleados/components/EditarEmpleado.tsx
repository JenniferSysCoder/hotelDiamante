...
export function EditarEmpleado() {
    ...
    useEffect(() => {
        const obtenerEmpleado = async () => {
            const response = await fetch(`${appsettings.apiUrl}Empleados/Obtener/${id}`);
            if (response.ok) {
    ...
    const guardar = async () => {
        if (empleado.idHotel === 0) {
            Swal.fire("Faltan datos", "Seleccione un hotel", "warning");
            return;
        }

        const response = await fetch(`${appsettings.apiUrl}Empleados/Editar/${empleado.idEmpleado}`, {
            method: "PUT",
        ...
        if (response.ok) {
            ...
            Swal.fire("Error", errorData.mensaje || "No se pudo actualizar el empleado", "error");
        }
    };
