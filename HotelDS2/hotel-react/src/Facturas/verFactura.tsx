  ...
  useEffect(() => {
    const obtenerFactura = async () => {
      const response = await fetch(
        `${appsettings.apiUrl}Facturas/Obtener/${id}`
      );
      if (response.ok) {
      ...
      <Card className="shadow border-0" style={{ width: "700px" }}>
        <CardBody>
          <div ref={printRef} style={{ width: "100%" }}>
            {/* Encabezado */}
            <div
              className="d-flex justify-content-between align-items-center border-bottom mb-4 pb-2"
              style={{ borderColor: "#d32f2f" }}
            >
              <div>
                <h1 style={{ color: "#d32f2f", marginBottom: "5px" }}>
                  Hotel Diamante
                </h1>
                <p className="mb-0">
            ...
            <div className="info">
              <strong>Factura No:</strong> {factura.idFactura}
            </div>
            <div className="info">
            ...
            <div className="info">
              <strong>Servicio:</strong> {factura.nombreServicio}
            </div>
